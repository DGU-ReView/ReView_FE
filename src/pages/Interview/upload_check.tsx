    ...
    <InterviewLayout activeMenu="upload">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <div className="flex items-center justify-end gap-4 mb-2">
          <span className={`text-lg ${interviewType === 'normal' ? 'font-semibold text-gray-900' : 'text-gray-500'}`}>
            일반면접
          </span>
          <button
            onClick={() => setInterviewType(interviewType === 'normal' ? 'pressure' : 'normal')}
            className={`relative w-14 h-7 rounded-full transition-colors ${
              interviewType === 'pressure' ? 'bg-gray-800' : 'bg-gray-300'
            }`}
          >
            <div
          ...
          <span className={`text-lg ${interviewType === 'pressure' ? 'font-semibold text-gray-900' : 'text-gray-500'}`}>
            압박면접
          </span>
        </div>
        </div>
        ...
        <div className="w-full max-w-lg mb-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700">2025년_3월_자소서</span>
            </div>
            <button 
              onClick={handleJobSelect}
              className="bg-sky-400 hover:bg-sky-500 text-white px-6 py-2 rounded-lg transition-colors"
            >
              직무선택
            </button>
          </div>
