...
export const getOtherInterview = async (): Promise<TEvaluateOtherResponse> => {
  const { data } = await axiosInstance.get('/api/peer-reviews/random');
  return data;
};
export const postOtherInterview = async ({ recordingId, body, followUpQuestion }: TEvaluateOtherRequest): Promise<TEvaluateresponse> => {
  const { data } = await axiosInstance.post(`/api/peer-reviews/recordings/${recordingId}/feedbacks`, { body, followUpQuestion });
  return data;
};
