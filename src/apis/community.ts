...
export const getCommunityList = async (param: TCommumityListRequest): Promise<TCommumityListResponse> => {
  const { data } = await axiosInstance.get<TCommumityListResponse>('/api/community/pages', {
    params: param,
  });
  return data;
};
export const postCommunity = async (param: TCommunityPostRequest): Promise<TCommumityListResponse> => {
  const { data } = await axiosInstance.post<TCommumityListResponse>('/api/community/pages', param);
  return data;
};
export const getCommunityDetail = async ({ pageId }: TCommumityDetailRequest): Promise<TCommumityDetailResponse> => {
  const { data } = await axiosInstance.get<TCommumityDetailResponse>(`/api/community/pages/${pageId}`);
  return data;
};
export const updateCommunity = async (param: TCommunityEditRequest): Promise<TCommumityDetailResponse> => {
  const { data } = await axiosInstance.patch<TCommumityDetailResponse>(`/api/community/pages/${param.pageId}`, param);
  return data;
};
